...
plotProgress <- function(name, output) {
    conn <- file(name, 'r')
    header <- readLines(conn, 1)
    K <- as.integer(header)
    d <- read.csv(conn, header=T)

    # print(d)
    # melted <- melt(d, id='iter')
    # ggplot(data=melted, aes(x=i, y=value, colour=variable)) +
    #     geom_line()

    pl <- ggplot(data=d, aes(x=i, y=cost, colour=cost, group=iter)) +
        geom_line() + theme_bw() + labs(title = K)
    # print(K)
    # print(d)
    ggsave(plot=pl, file=output, width=12, height=6)
}

plotProgress(problem, output)
